...
def setup_logging(resource: Resource):
    try:
        ...
        # Create logger provider with error handling
        try:
            logger_provider = LoggerProvider(resource=resource)
            set_logger_provider(logger_provider)
            logger_provider.add_log_record_processor(BatchLogRecordProcessor(log_exporter))
        except Exception as e:
            logging.error(f"Failed to create logger provider: {str(e)}")
            return
        ...
        try:
            ...
            logging.error(f"Failed to add OpenTelemetry handler: {str(e)}")
            return
        ...
        try:
            ...
            logging.error(f"Failed to instrument logging: {str(e)}")
            return
